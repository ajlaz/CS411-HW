...
def create_meal(meal: str, cuisine: str, price: float, difficulty: str) -> None:
    ...
    Raises:
        ...
        sqlite3.IntegrityError: If a meal with the same meal name exists.
...
def get_leaderboard(sort_by: str="wins") -> dict[str, Any]:
    """
    Returns the leaderboard of meals, sorted either by wins or win percentage (wins * 1.0 / battles)
    
    Args:
        sort_by (str): Which column to sort by (either "wins" or "win_pct").
        
    Returns:
        dict (dict[str, any]): A dictionary representing the sorted leaderboard of non-deleted meals.
...
def update_meal_stats(meal_id: int, result: str) -> None:
    """
    Updates the stats of a meal based on whether it won or lost a battle.
    
    Args:
        meal_id (int): The ID of the meal to update.
        result (str): The result of the battle to use for updating stats (either "win" or "loss").
    
    Raises:
        ValueError: If the song does not exist, is marked as deleted, or an invalid result is passed.
